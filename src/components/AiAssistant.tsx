...
export function AiAssistant() {
  ...
  const handleBotResponse = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString(), type: "bot", text: data.content }]);
    } catch (error) {
      setMessages((prev) => [...prev, { id: Date.now().toString(), type: "bot", text: "Sorry, something went wrong." }]);
    } finally {
      setIsTyping(false);
    }
  };
          ...
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.type === "user" ? "justify-end" : "justify-start"}`}>
                  {msg.type === "bot" && <Sparkles className="w-5 h-5 text-primary mr-2" />}
                  <div className={`p-3 rounded-lg ${msg.type === "user" ? "bg-primary text-white" : "bg-gray-200 dark:bg-gray-700"}`}>
                    {msg.text}
                  </div>
                </div>
              ))}
            ...
            <div className="p-4 border-t flex gap-2">
              <Input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && handleSendMessage(inputValue)} placeholder="Ask a question..." className="flex-1" />
